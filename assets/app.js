...
function renderChrome(){
  ...
  topbar.innerHTML = `
    ...
    <a class="brand" href="index.html"><span class="brand-mark">SY</span><span class="brand-name">${SITE.name}</span></a>
    ...
    <a class="top-link" href="${SITE.officialUrl}" target="_blank" rel="noreferrer"><span>学校官网</span>↗</a>
    <a class="top-link" href="${SITE.repoUrl}" target="_blank" rel="noreferrer"><span>GitHub</span>↗</a>`;
  sidebar.innerHTML = `
...
function initTheme(){
  const saved=localStorage.getItem("syuct-guide-theme");
  const preferred=matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light";
  setTheme(saved||preferred);
}
function setTheme(theme){
  document.documentElement.dataset.theme=theme;
  localStorage.setItem("syuct-guide-theme",theme);
}
...
document.addEventListener("keydown",e=>{
  ...
  if(e.key==="Escape"){closeSearch();closeGroupModal();document.getElementById("lightbox")?.classList.remove("open");document.body.classList.remove("modal-open");}
});
document.addEventListener("DOMContentLoaded",()=>{
  initTheme();renderChrome();renderSearch();renderGroupModal();initGroupModal();initLightbox();initResourceFilter();
  document.getElementById("notFoundSearch")?.addEventListener("click",openSearch);
});
